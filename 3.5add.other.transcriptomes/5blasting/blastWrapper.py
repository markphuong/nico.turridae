...
def align(element):
	...
	blastoutput = element + '.phylo.blastoutput',
	db = 'Lottia_gigantea.GCA_000327385.1.26.pep.all.fa',
	index = element
	) #name your output


	commands = """
	makeblastdb -dbtype nucl -in {trinity}
	tblastn -query {db} -db {trinity} -outfmt 6 -out {index}.tblastnoutput -evalue 1e-10
	blastx -query {trinity} -db {db} -outfmt 6 -out {blastoutput} -evalue 1e-10
	""".format(**variables)
