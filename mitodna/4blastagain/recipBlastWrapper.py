...
def align(element):
	...
	commands = """
	cp /pylon5/bi4s86p/phuong/nico.turridae/mitodna/2mapping/{sample}*.vcf $LOCAL
	cp /pylon5/bi4s86p/phuong/nico.turridae/mitodna/1blasting/{sample}_*.NI $LOCAL
	makeblastdb -dbtype nucl -in {sample}_assemblies_clustered.fasta.NI
	blastn -query {sample}_assemblies_clustered.fasta.NI -db lophigenome -outfmt 6 -out {sample}.blastoutput -evalue 1e-10
	blastn -query lophigenome -db {sample}_assemblies_clustered.fasta.NI -outfmt 6 -out {sample}.recipblastoutput -evalue 1e-10
	python makeVCFcov.py {sample}_assemblies_clustered.fasta.NI {sample}.recipblastoutput {sample}_5.1.vcf {sample}.cov
	python contig_filter.v3.py {sample}.cov {sample}_assemblies_clustered.fasta.NI {sample}.recipblastoutput {sample}
	cp {sample}.blastoutput /pylon5/bi4s86p/phuong/nico.turridae/mitodna/4blastagain
	cp {sample}.recipblastoutput /pylon5/bi4s86p/phuong/nico.turridae/mitodna/4blastagain
	cp {sample}.cov /pylon5/bi4s86p/phuong/nico.turridae/mitodna/4blastagain
	cp {sample}_removed /pylon5/bi4s86p/phuong/nico.turridae/mitodna/4blastagain
	cp {sample}_filtered_recipblast /pylon5/bi4s86p/phuong/nico.turridae/mitodna/4blastagain
	""".format(**variables)
